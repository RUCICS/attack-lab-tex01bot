...
这四道题做下来，我最大的感受是：漏洞利用是一个不断在读汇编—还原运行时栈状态—选择策略—验证自己的猜测之间来回反复的过程。尤其是从 Problem 2 开始，只要题目稍微换一个细节（有没有 gadget、参数需不需要传+怎么传、有没有防护），解法就会完全变样。
...
所以这题给我的收获不是“我会绕过 canary 了”，而是更具有启发性的一点：  
